...
import { drainSignals, postSignal, SignalType } from '../signals.js';
...
/**
 * Process any pending PROFIT_REALIZED signals from the Sniper Bot.
 * Deposits the profit ETH amount into the Aave vault automatically.
 */
async function processIncomingSignals() {
    const pending = drainSignals(SignalType.PROFIT_REALIZED);
    for (const signal of pending) {
        const { tokenSymbol, pnlEth } = signal.data;
        addLog('info', `📡 Signal received from Sniper Bot: PROFIT_REALIZED (+${Number(pnlEth).toFixed(6)} ETH from ${tokenSymbol})`);
        try {
            await depositToVault('ETH', pnlEth);
            postSignal(SignalType.VAULT_DEPOSIT_DONE, { pnlEth, tokenSymbol, signalId: signal.id });
            addLog('success', `✅ Auto-deposited +${Number(pnlEth).toFixed(6)} ETH profit into Aave vault`);
        } catch (err) {
            postSignal(SignalType.VAULT_DEPOSIT_FAILED, { pnlEth, tokenSymbol, error: err.message, signalId: signal.id });
            addLog('error', `Auto-deposit failed for signal #${signal.id}: ${err.message}`);
        }
    }
}
...
export async function getVaultStatus() {
    // Process any profit signals from the Sniper Bot before returning status
    if (process.env.RPC_URL && process.env.WALLET_PRIVATE_KEY) {
        await processIncomingSignals().catch((err) => {
            addLog('error', `Signal processing error: ${err.message}`);
        });
    }
...
export async function depositToVault(asset, amount) {
    ...
    try {
        ...
        if (asset === 'ETH') {
            ...
            vaultPositions.set('ETH', { depositedCost: prev.depositedCost + Number(amount) });
        } else {
            const tokenAddresses = {
                USDC: '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48',
                USDT: '0xdAC17F958D2ee523a2206206994597C13D831ec7',
            ...
            vaultPositions.set(asset, { depositedCost: prev.depositedCost + Number(amount) });
        }
