...
import { postSignal, SignalType } from '../signals.js';
    ...
    for (const [tokenAddress, position] of positions) {
        ...
        if (shouldSell) {
            try {
                ...
                // ── Signal the Vault Bot so it can auto-compound profits ──────────
                if (pnl > 0) {
                    postSignal(SignalType.PROFIT_REALIZED, {
                        tokenSymbol: position.tokenSymbol,
                        tokenAddress,
                        pnlEth: pnl,
                        txHash: result.txHash
                    });
                    addLog('info', `📡 Signal sent to Vault Bot: PROFIT_REALIZED (+${pnl.toFixed(6)} ETH)`);
                }
