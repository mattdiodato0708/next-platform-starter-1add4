...
/** SSE subscriber controllers (one per connected client). */
...
/** Push the current status to all SSE subscribers. */
function pushStatus() {
 ...
 */
export function subscribeSSE(controller) {
    sseControllers.add(controller);
    // Send current state immediately
    try {
        controller.enqueue(`data: ${JSON.stringify(getStatus())}\n\n`);
    } catch (_) {}
    return () => sseControllers.delete(controller);
}
