...
async function executeBuy(tokenAddress, ethAmount, slippagePct) {
    ...
    const gasPrice = (feeData.gasPrice * gasMultBps) / 10000n;
...
export function startBot(userConfig = {}) {
    ...
    if (!process.env.RPC_URL) {
        return { started: false, message: 'RPC_URL environment variable is not set' };
    }
    if (!process.env.WALLET_PRIVATE_KEY) {
        return { started: false, message: 'WALLET_PRIVATE_KEY environment variable is not set' };
    }
